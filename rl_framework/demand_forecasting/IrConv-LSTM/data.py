...
class escooter_1h_9h3_STG(Dataset):
    def __init__(
        ...
        count = 0
        for index in range(
            date_to_index_dict[start_date] - 14 * 24,
            date_to_index_dict[self.end_date] + 1,
        ):
            self.raw_data_collection.append(
                self.zeros_grids_new[index_to_date_dict[index]]
            )
        ...
        self.chunks_closeness = (
            torch.FloatTensor(np.array(self.closeness_array))
            .squeeze(1)
            .unfold(0, self.closeness, self.closeness)
            .permute(0, 3, 1, 2)
        )
        self.chunks_period = (
            torch.FloatTensor(np.array(self.period_array))
            .squeeze(1)
            .unfold(0, self.period, self.period)
            .permute(0, 3, 1, 2)
        )
        self.chunks_trend = (
            torch.FloatTensor(np.array(self.trend_array))
            .squeeze(1)
            .unfold(0, self.trend, self.trend)
            .permute(0, 3, 1, 2)
        )
        self.chunks_target = (
            torch.FloatTensor(np.array(self.target)).unsqueeze(1).unsqueeze(3)
        )
...
def date_to_index(start_time, end_time):
    ...
    for index, item in enumerate(date_hourly_interval):
        key = (
            str(item).split(" ")[0].split("-")[1]
            + str(item).split(" ")[0].split("-")[2]
