...
def data_saving_2km(gts_in, predcitions_in):
    ...
    for index, line_no in enumerate(node_list):
        ...
        for day in range(len(gts_in)):
            f_out.writelines(
                [
                    str(predcitions_in[day][index][0]),
                    ",",
                    str(gts_in[day][index][0]),
                    ";",
                ]
            )
        f_out.write("\n")
...
if __name__ == "__main__":
    ...
    gts_array_reverse = maxminscaler_3d_reverse(
        gts_array, dtest.get_max_min()[0], dtest.get_max_min()[1]
    )
    predictions_array_reverse = maxminscaler_3d_reverse(
        predictions_array, dtest.get_max_min()[0], dtest.get_max_min()[1]
    )
