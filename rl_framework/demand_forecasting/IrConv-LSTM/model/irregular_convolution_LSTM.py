...
class irregular_convolution(nn.Module):
    ...
    def forward(self, x_input):
        ...
        x_size = x_input.size()
        x_input = x_input.permute(0, 1, 3, 2).reshape(
            [x_size[0], x_size[2], reshape_size]
        )
        if self.bias:
...
class Extraction_spatial_features(nn.Module):
    ...
    def prepare_data(self, input_x):
        ...
        temporal_out = (
            torch.masked_select(temporal_out, self.mask)
            .reshape([shape[0], shape[1], shape[3], self.kernel_size])
            .cuda()
        )
        return temporal_out
    ...
    def forward(self, input):
        ...
        for index in range(self.seq_len):
            ...
            cnn_output = self.relu(
                self.batchnormal(self.irregular_layer2(cnn_input))
            ).permute(0, 1, 3, 2)
            cnn_input = self.prepare_data(input_x=cnn_output)
...
class Irregular_Convolution_LSTM(nn.Module):
    def __init__(
        ...
        self.convlstm_trend = Convolution_LSTM(
            input_channels=125, hidden_channels=125, num_layer=2, batch_size=self.bsize
        ).cuda()
...
def reconstruction_file(kernel_size):
    ...
    :param similarity_csv_path: path to CSV file, must have
                            first column as row labels and
                            a header row of column labels.
    :param kernel_size: number of neighbors to select per node.
    :return: zero_matrix: np.ndarray of shape (N, N), dtype=int.
    """
    # 1) Load the full similarity dataframe
    df = pd.read_csv(
        "/home/ruroit00/rebalancing_framework/rl_framework/demand_forecasting/IrConv-LSTM/data/similarity_matrix.csv",
        index_col=0,
    )

    # 2) Build a mapping from node-id to row index
    ...
    for node in node_ids:
        sim_row = df.loc[node]
        # Exclude self by dropping it
        sim_row = sim_row.drop(node)
        # Pick the labels of the top-k highest similarities
        topk_nodes = sim_row.nlargest(kernel_size).index.tolist()

        i = node_to_idx[node]
        for neigh in topk_nodes:
            j = node_to_idx[neigh]
            mask[i, j] = 1
