...
def train_model(size_of_kernel=1):  # Set default kernel size to 1 for hexagonal grid
    ...
    model = irconvlstm(
        input_size_closeness=closeness_size,
        input_size_period=period_size,
    ...
    for i in range(max_epochs):
        ...
        if use_cuda:
            data1_val, data2_val, data3_val, labels_val = (
                Variable(data1_val.cuda()),
                Variable(data2_val.cuda()),
                Variable(data3_val.cuda()),
                Variable(labels_val.cuda()),
            )
        else:
            data1_val, data2_val, data3_val, labels_val = (
                Variable(data1_val),
        ...
        avg_losses_epoch_valid = sum(losses_epoch_valid) / float(
            len(losses_epoch_valid)
        )
        ...
        print(
            ...
            " ",
            "validation_MAE = ",
            avg_losses_epoch_valid.cpu().detach().numpy(),
        )
        losses.append(loss_)
...
if __name__ == "__main__":
    # size of kernel represents number of semantic neighbors
